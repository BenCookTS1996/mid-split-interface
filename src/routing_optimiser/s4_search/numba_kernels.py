...
def make_numba_eval(M, ref, zr, zq, mid_id, profile_starts, profile_counts, elig,
    ...
    # EXACTLY per generation OUTSIDE the kernel (seed_search eval wrapper + band_scoring), so
    ...
    from routing_optimiser.s4_search.seed_search import _mid_viol_weights
    ...
    # Breach penalty (must match seed_search._obj_viol._pen): fixed hit + quadratic overage.
    ...
    # MUST match seed_search._obj_viol._pen — verify() cross-checks and falls back on mismatch.
