...
def detect_blocked_gateways(adf, min_consecutive: float, date_col: str = "date"):
    ...
    g = (d.groupby(["_bin", "_gw", "_day"], as_index=False)
    ...
    g = g.sort_values(["_bin", "_gw", "_day"], ascending=[True, True, False])
    g["_cmax"] = g.groupby(["_bin", "_gw"])["suc"].cummax()
    _run = (g[g["_cmax"] <= 0].groupby(["_bin", "_gw"], as_index=False)["att"].sum()
            .rename(columns={"att": "consec_failed"}))
    out = g[["_bin", "_gw"]].drop_duplicates().merge(_run, on=["_bin", "_gw"], how="left")
    out["consec_failed"] = out["consec_failed"].fillna(0.0)
    _succ = (g[g["suc"] > 0].groupby(["_bin", "_gw"], as_index=False)["_day"].max()
             .rename(columns={"_day": "last_success_date"}))
    out = out.merge(_succ, on=["_bin", "_gw"], how="left")
    out["blocked"] = out["consec_failed"] >= float(min_consecutive)
    out = out.rename(columns={"_bin": "bin", "_gw": "gateway"})
