-- MASTERCARD monthly-renewal daily billing weights (parallels queries/mr_weights.sql).
-- Counts MASTERCARD Monthly-Renewal transactions per day-of-month to build the front-loaded
-- daily CDF used to pro-rate mid-month gateway kills. 12-month window from Month 0.
WITH rawQ AS (
  SELECT
      ...
      COUNT(DISTINCT CASE WHEN (
        ...
        AND NOT COALESCE(LEFT((
          CASE
            ...
            ELSE riskdata2025.accountType
          END), 6) = 'paypal', FALSE) 
      THEN riskdata2025.gatewayTransactionId ELSE NULL END) AS mastercard_trx_count
...
ShareData AS (
  SELECT
    rawQ.recordMonth,
    rawQ.recordDOM,
    rawQ.mastercard_trx_count,
    SUM(rawQ.mastercard_trx_count) OVER(PARTITION BY rawQ.recordMonth) AS monthTotal,
    rawQ.mastercard_trx_count / (SUM(rawQ.mastercard_trx_count) OVER(PARTITION BY rawQ.recordMonth)) AS monthShare,
    CASE
