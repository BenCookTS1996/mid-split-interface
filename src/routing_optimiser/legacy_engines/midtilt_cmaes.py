...
s4_search/ is the point: s4_search is what the live search uses.

WHY IT IS UNREACHABLE, in three links — check these before assuming this code runs:
  1. `engine_key` is ALWAYS "genetic_fullmatrix". tab 2 builds its engine dropdown from an
     empty list and appends exactly one option; the registry's `engine_choices()` does not feed it.
  2. tab 2 (`if _safe_G is None and engine_key == "genetic_fullmatrix"`) SKIPS the preliminary
     endpoint search outright and logs "no preliminary endpoint search is run". `run_midtilt_ga`
     is only reachable from the `elif` beneath it.
  3. Its other call site is guarded by `engine_key == "genetic_numba"`, a key that no longer
     exists in `choices`.

WHAT IT IS. A real, complete Active (mu/mu_w, lambda)-CMA-ES, hand-written in numpy — covariance
adaptation, two evolution paths, active negative recombination weights (Hansen 2016), sigma
control, IPOP restarts, epsilon-constrained feasibility-first ranking. No `cma` package involved.
The genome is COMPACT: 3 knobs per vampMid (risk-tilt, return-tilt, gain) applied as a tilt around
a reference split. That is fast, stable and hard-compliant, but it can only reach splits that are
tilts/scalings of the reference — which is why the full-matrix GA replaced it.

WHAT STAYED BEHIND, and why this file imports it rather than copying it. Five functions are shared
with the live search and remain the single definition in s4_search/seed_search.py:

    _obj_viol            the REFERENCE fitness. numba_kernels._fused_eval and band_scoring are
                         written to match it and verify() cross-checks against it — so it must
                         have exactly one definition, and it must be the live one.
    _breach_pen          the band penalty shape _obj_viol uses
    _mid_sums            per-vampMid share/risk sums
    _mid_viol_weights    per-band priority weights
    _build_mid_incidence the (vampMid x row) incidence matrix

Duplicating any of them would let the retired engine and the live one drift apart silently, which
is the specific failure this project has been bitten by before.

TO REVIVE: re-add a `("genetic", ...)` entry to tab 2's `choices`, and restore the `elif` path to
`_ga_solve_with_correction`. Nothing here needs to change.
"""
from __future__ import annotations

import numpy as np

# The five shared definitions live with the LIVE search. Import, never copy — see the header.
from routing_optimiser.s4_search.seed_search import (
    _breach_pen, _build_mid_incidence, _mid_sums, _mid_viol_weights, _obj_viol)

__build__ = "19gd-split-from-seed_search"
...
# ===========================================================================
# [FN-109] _ret_z_per_mid removed — the revenue axis (θq) now uses the shared `_z_per_mid` above.
...
def _violation_breakdown(shares, ctx, top_k=20):
    ...
    # [FN-119]
    def _pen(_ov):
        return _breach_pen(_ov, _bfix, _qwt, _pexp)          # shared def (see _obj_viol / numba kernel)
...
def _cmaes(eval_ov, x0, sigma0, lo, hi, *, popsize, max_iter, seed, stop_check=None,
           ...
           sigma_floor=0.0, damps_mult=1.0, viol_count=None):
    ...
    best_nv = None                                          # # unmet-band vampMids of the incumbent
    gen_trace = []
    ...
    for it in range(int(max_iter)):
        ...
        if skeys[_b] < best_key - 1e-12:
            ...
            if viol_count is not None:                     # recompute unmet-MID count ONLY on an
                try:                                       # improvement (rare) → negligible cost
                    best_nv = int(viol_count(best_x))
                except Exception:  # noqa: BLE001
                    best_nv = None
        else:
            _stall += 1
        if progress_cb is not None:                        # live count + best-so-far score (score = -key,
            # higher = better) + the FITNESS (objective/revenue-ish) of that incumbent split + the #
            # of vampMids with an unmet band on it. Degrades gracefully to 3-/2-/1-arg callbacks.
            _cur_fit = float(best_ov[0]) if np.isfinite(best_ov[0]) else float("nan")
            for _pargs in ((int(lam), float(-best_key), _cur_fit, best_nv),
                           (int(lam), float(-best_key), _cur_fit),
                           (int(lam), float(-best_key)),
                           (int(lam),)):
                try:
                    progress_cb(*_pargs)
                    break
...
def run_midtilt_ga(ctx, *, pop_size=40, generations=80, seed=42,
    ...
    if _exact_bands is not None and _band_inc is not None:
        ...
        # [FN-128b] Count DISTINCT vampMids with an unmet band on a unit-cube genome `v` (the
        # CMA-ES incumbent). Uses the SAME projector as the fitness, so the live count reconciles
        # with the score. Returns an int; safe on any degenerate band (skips it).
        def _viol_count(v):
            X = _decode(_to_actual(np.asarray(v, float)[None, :]))
            mids = set()
            for _r in _exact_bands.report(_s2pr(X, _band_inc)):
                _now = float(_r["now"])
                if ((_r["ceil"] is not None and _now > float(_r["ceil"]) + 1e-6)
                        or (_r["floor"] is not None and _now < float(_r["floor"]) - 1e-6)):
                    mids.add(_r["midl"])
            return len(mids)
    else:
        # [FN-129]
    ...
    for r in range(n_r):
        ...
        bx, bk, bov, tr, fpop = _cmaes(eval_ov, start, s0 * _s0m, np.zeros(D), np.ones(D),
                                       ...
                                       progress_cb=progress_cb, viol_count=_viol_count,
    ...
    # [FN-147] (reuses the seeding feasibility-first key `_seed_key` — identical ordering)
    _ordered = sorted(restart_bests, key=lambda t: _seed_key(t[1]))
    _arch = [best_G[0].copy()]
