-- FP&A macro forecast (rpgt units per period). Shared source with the Visa pipeline; the
-- Mastercard share is applied downstream via mapping_pct. Replaces the notebook's 'FcastData'
-- Google Sheet with the BigQuery FP&A table (headless, mirrors the VAMP pipeline).
